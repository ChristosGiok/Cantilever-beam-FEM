...
from Input.element import Element
import numpy
...
param_string = input("Enter coma separated values of s,p,xp,w,E,b,d in given order for cantilever beam: ")
s, p, xp, w, E, b, d = param_string.split(",")

s = float(s.strip()) #span of beam
p = float(p.strip()) #Applied load
p = -p
xp = float(xp.strip()) #Distance of applied load from fixed end
w = float(w.strip()) #load intensity
w = -w
E = float(E.strip()) #Elastic modulus
b = float(b.strip()) #width of beam's cross section
d = float(d.strip()) #depth of beam's cross section
EI = E * b*d**3/12

'''--------------------------Main General Methods------------------------'''
def elem(nodes):
    """Creates element from two nodes,
    It takes a list of nodes and returns a list of elements"""
    el = []
